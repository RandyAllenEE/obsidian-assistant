...
export interface DocumentSelectorCompatLayer {
    getAdditionalDocumentSelectorStringForFolder?(folderName: string, pluginSettings: MyFoldersSettings): string;
    shouldMutationRecordTriggerFolderReProcessing?(record: MutationRecord): boolean;
}
...
export const CompatQuickExplorer: DocumentSelectorCompatLayer = {
    getAdditionalDocumentSelectorStringForFolder: function (folderName: string, pluginSettings: MyFoldersSettings): string {
        if (folderName.toLowerCase().startsWith("endswith::")) {
